...
def destiny2_api_public(url, api_key):
    ...
    #response2 = requests.get(url2, headers=my_headers)
...
BB=requests.get('https://www.bungie.net' , params='en/OAuth/Authorize')
CC=requests.get('https://www.bungie.net' , params='Platform/App/OAuth/token/')

A=requests.get('https://www.bungie.net/en/OAuth/Authorize')
B=requests.get('https://www.bungie.net/Platform/App/OAuth/token/')

DD=requests.get('https://www.bungie.net' , params='ReadBasicUserProfile')
C='ReadBasicUserProfile'


def destiny2_api_private(url, api_key):
    """This is the main function for everything. It requests the info from the bungie servers
    by sending a url."""
    my_headers = my_headers = {"X-API-Key": my_api_key}
    response = requests.get(url, headers = my_headers)
...
def main():
    # my_api_key='afb7b0fcc0604ab49612af8de1b758f2'
    # user_name='Chobofied'
    # user_name='C6275913'
    # user_playform=
